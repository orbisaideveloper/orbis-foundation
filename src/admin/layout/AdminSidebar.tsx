...
const AdminSidebar: React.FC = () => {
  return (
    <aside className="w-full md:w-72 shrink-0 flex flex-col bg-white/60 backdrop-blur-xl border-b md:border-b-0 md:border-r border-white shadow-lg md:h-full" aria-label="Admin Navigation">
      {/* Desktop Header */}
      <div className="hidden md:block p-8 border-b border-white">
        <h2 className="text-3xl font-black text-slate-800 tracking-widest uppercase flex flex-col">
          <span>ORBIS</span> 
          <span className="text-green-600 text-sm font-bold tracking-[0.4em] mt-1">FOUNDATION</span>
        </h2>
      </div>
      ...
      <nav className="p-4 overflow-x-auto md:overflow-y-auto flex md:flex-col gap-3 flex-1 scrollbar-hide">
        {navItems.map((item) => (
          ...
          >
            ...
            <span className="text-sm font-bold tracking-wide whitespace-nowrap">{item.label}</span>
          </NavLink>
        ))}
      </nav>
      ...
      <div className="hidden md:block p-6 border-t border-white text-xs font-bold text-slate-500 text-center bg-slate-50/50 mt-auto">
        <span className="text-green-500 animate-pulse">●</span>{' '}Zero Mock Data Policy Enforced
      </div>
    </aside>
  );
};
