...
describe('Engine', () => {
    ...
    it('should handle initialization failure', async () => {
        ...
        vi.spyOn(adapterManager, 'initializeAll').mockRejectedValueOnce(new Error('Test Error'));
        
        await expect(engine.initialize({ environment: 'test', version: '1.0.0' })).rejects.toThrow('Test Error');
        expect(engine.getStatus()).toBe('STOPPED');
    });
});
