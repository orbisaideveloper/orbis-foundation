...
try {
  ...
  console.log('⚠️ Prisma Client require path issue, skipping direct DB sync execution.');
  process.exit(0);
}
...
async function syncLogs() {
  try {
    ...
    for (const block of blocks) {
      ...
      if (!existing) {
        const filesRaw = execSync(`git show --name-only --format="" ${hash}`, { cwd: rootPath }).toString();
